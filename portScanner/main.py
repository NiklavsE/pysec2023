...
##########################################################################
# Script that performs a port scan on the provided IP address.
# The address is scanned for the 1000 most popular ports. (see ports.txt)
# Port scanning is done via multiprocessing.
# Example usaage in CLI:
# python main.py <your-ip>
##########################################################################
...
def doScan(target):
    ...
    results = [pool.apply_async(tcpScan, (target, chunkedPorts))
               for chunkedPorts in ports]
    openPorts = []
